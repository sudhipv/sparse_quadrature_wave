...
if(ip ==0):
    print('number of quadrature points',nQdpts)
    NP = 159
    chunks = int(nQdpts/NP)
    print('chunks is ', chunks)
    remnQD = nQdpts%NP
    print('remaning QD pts is ', remnQD)
else:
    chunks = None
    NP = None
    remnQD = None
...
remnQD = comm.bcast(remnQD, root = 0)


u_t = np.zeros([ndof,nt+1])
u_PC = np.zeros([ndof,nt+1,PCE_u])
u_PCsum = np.zeros([ndof,nt+1,PCE_u])
...
#     spath = './u_nisp_mean_beforeReduce.mat'
#     sio.savemat(spath, {'u_nisp_mean_befRed':u_PCsum})

comm.Reduce(u_PCsum, mu_sum, op=MPI.SUM, root=0)


if(ip == 0):

    u_pdf = np.zeros([9,PCE_u])

    norm = normpc(dim_L,PCE_u, mIndex)

    ####### Output normalization when all the chaos basis functions are normalized by their standard deviation #####
    ###### Input is normalized in the log normal expansion itself for coefficeints #######
    ...
    u_pdf[0,0:PCE_u] = mu_sum[400,149,:]
    u_pdf[1,0:PCE_u] = mu_sum[400,299,:]
    u_pdf[2,0:PCE_u] = mu_sum[400,461,:]

    u_pdf[3,0:PCE_u] = mu_sum[401,149,:]
    u_pdf[4,0:PCE_u] = mu_sum[401,299,:]
    u_pdf[5,0:PCE_u] = mu_sum[401,461,:]

    u_pdf[6,0:PCE_u] = mu_sum[402,149,:]
    u_pdf[7,0:PCE_u] = mu_sum[402,299,:]
    u_pdf[8,0:PCE_u] = mu_sum[402,461,:]
    ...
    # spath = './u_nisp_pc.mat'
    # sio.savemat(spath, {'u_nisp_pc':mu_sum})
